...
PATH <- "RESPINOW/nowcasts_retrospective/"
...
df_frozen_truth <- load_frozen_truth("seasonal_influenza", LEAD_TIME)
...
if (LEAD_TIME == 0) {
  ...
  t <- paste0("-", LEAD_TIME, " week ahead inc hosp")
}
...
ggplot(df1, aes(x = target_end_date)) +
  ...
  labs(x = NULL, y = "7-day case incidence", title = paste("Horizon:", LEAD_TIME, "weeks")) +
  scale_alpha_manual(
...
ggsave(paste0("RESPINOW/figures/nowcasts_", LEAD_TIME, "w.pdf"), width = 164, height = 100, unit = "mm", device = "pdf")
